...
import os
rootDir='C:/Users/Administrator/Desktop/标签体系/数据预处理/taskFiles'
...
#            file_task_1='C:/Users/Administrator/Desktop/标签体系/数据预处理/tasks_tml.xlsx'
            ...
            FrameMessage=FrameTask_0[['appid','title','url','desc']].loc[DiffDataIndex].copy()
            Frame=DataFrame([FrameTaskNotNullHow_0['场景'].loc[DiffDataIndex], FrameTaskNotNullHow_1['场景'].loc[DiffDataIndex],FrameTaskNotNullHow_0['行业'].loc[DiffDataIndex],
                           FrameTaskNotNullHow_1['行业'].loc[DiffDataIndex]],index=['0_场景','1_场景','0_行业','1_行业'])
            Frame=Frame.append(FrameMessage.T)
            
            #Frame['appid','title','url','desc']=FrameMessage
            Frame=Frame.T
            Frame['序号']=DiffDataIndex+2
...
#            print ("不同个数:",len(Frame))
            if flag==0:
                Frame.to_csv('C:/Users/Administrator/Desktop/标签体系/数据预处理/ProcessResult/dataComparison.csv', index=False,mode='w')
                flag=1
            else:
                Frame.to_csv('C:/Users/Administrator/Desktop/标签体系/数据预处理/ProcessResult/dataComparison.csv', index=False,mode='a',header=False)
